...
htplot <- ggplot(ht, aes(x=-log(FDR.q.val), y=reorder(NAME, -log(FDR.q.val)), fill = NES))
...
hwplot <- ggplot(hw, aes(x=-log(FDR.q.val), y=reorder(NAME, -log(FDR.q.val)), fill = NES))
...
hrplot <- ggplot(hr, aes(x=-log(FDR.q.val), y=reorder(NAME, -log(FDR.q.val)), fill = NES))
...
gseaplot <- ggplot(sel, aes(x=-log(FDR.q.val), y=reorder(NAME, -log(FDR.q.val)), fill = NES))
...
gseaplot <- ggplot(rdt, aes(x=-log(FDR.q.val), y=reorder(NAME, -log(FDR.q.val)), fill = NES))
gseaplot + geom_point(size=5, shape=21) + 
  geom_vline(xintercept = 2, linetype = "dashed", color = "red") + 
  theme(
    panel.background = element_rect(fill = "white", color = "black"), 
  ...
  scale_fill_gradient2(low="blue", mid="white", high="red", midpoint=0, limits=c(-2.5,2.5)) + 
  labs(x="-log(FDR q-value)", y="Reactome Pathways", title="Normalized Enrichment Score")
```
...
mtplot <- ggplot(mt, aes(x=-log(FDR.q.val), y=reorder(NAME, -log(FDR.q.val)), fill = NES))
...
mwplot <- ggplot(mw, aes(x=-log(FDR.q.val), y=reorder(NAME, -log(FDR.q.val)), fill = NES))
...
mrplot <- ggplot(mr, aes(x=-log(FDR.q.val), y=reorder(NAME, -log(FDR.q.val)), fill = NES))
