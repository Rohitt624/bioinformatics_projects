...
```{r}
all <- read.csv("C:/Users/rthalla/OneDrive - Loyola University Chicago/Zhang Lab/Kanak Tet2 Ripk3/dkotet2all.csv")
all <- all[which(all$NES > 1 | all$NES < -1),]
gseaplot <- ggplot(all, aes(x=FDR.q.val, y=reorder(NAME, NES), fill = NES))
gseaplot + geom_point(size=3, shape=21) + 
  geom_vline(xintercept = 0.1, linetype = "dashed", color = "red") + 
  theme(
    panel.background = element_rect(fill = "white", color = "black"), 
    plot.title = element_text(hjust = 0.5)
