...
VlnPlot(ery, features = c("ENG"))
VlnPlot(so, features = c("MAP3K7"), split.by = "dataset")
RidgePlot(ery, features = c("ENG"))
...
# Rescale expression within each gene (0–1 per gene)
...
genes <- c("GATA1", "SLAMF1", "ENG", "TFRC", "HBB", "ITGA2B")

# Create cluster-dataset identifier
ery$cluster_dataset <- paste0("Cluster", ery$seurat_clusters, "_", ery$dataset)

# get the unique labels that actually exist
labs <- unique(ery$cluster_dataset)

# extract the cluster numbers for sorting
clust_num <- as.numeric(sub("Cluster([0-9]+).*", "\\1", labs))

# extract condition (_AAVS or _SF3B1)
cond <- sub(".*(_AAVS|_SF3B1)", "\\1", labs)

# order: first by cluster number, then by condition
ord <- order(clust_num, cond)

# reassign factor with correct order
ery$cluster_dataset <- factor(ery$cluster_dataset, levels = labs[ord])

expr_data <- FetchData(
  ery,
  vars = c("seurat_clusters", genes)
)

# 2. Average expression per cluster_dataset
avg_expr <- expr_data %>%
  group_by(seurat_clusters) %>%
  summarise(across(all_of(genes), mean))

# convert to matrix with cluster_dataset as rownames
mat <- as.matrix(avg_expr[,-1])
rownames(mat) <- avg_expr$seurat_clusters

# 3. Plot heatmap
pheatmap(mat,
         cluster_rows = FALSE,   # keep your manual ordering
         cluster_cols = FALSE,    # cluster genes if you like
         scale = "column",
         color = colorRampPalette(c("blue", "white", "red"))(100),
         treeheight_row = 0,
         treeheight_col = 0)          # optional: normalize per gene
