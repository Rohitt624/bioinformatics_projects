...
Idents(so) <- "dataset"
so_sf3b1markers <- FindMarkers(so, ident.1 = "SF3B1", ident.2 = "AAVS", logfc.threshold = 0)
...
ery_highlight_data <- erymarkers[rownames(erymarkers) %in% genes_to_highlight, , drop = FALSE]
...
hsc_highlight_data <- hscmarkers[rownames(hscmarkers) %in% genes_to_highlight, , drop = FALSE]
...
ggsave("erymodulescore.png", plot = ery_hm, dpi = 600)
