...
ggsave("ery_volcano2.png", plot = eryvol, dpi = 600)
...
ggsave("hsc_volcano2.png", plot = hscvol, dpi = 600)

```

```{r TAK1 FC}
mk_sf3b1$neglog10p <- -log10(mk_sf3b1$p_val_adj)
gmp_sf3b1$neglog10p <- -log10(gmp_sf3b1$p_val_adj)
mast_sf3b1$neglog10p <- -log10(mast_sf3b1$p_val_adj)
eobp_sf3b1$neglog10p <- -log10(eobp_sf3b1$p_val_adj)
...
ggsave("tak1fc2.png", tak1, dpi = 600)
...
ggsave("gata1fc2.png", gata1, dpi = 600)
...
c <- cfue_sf3b1["TFRC", ]
p <- proe_sf3b1["TFRC",]
TFRC_ery <- rbind(m, b, c, p)
rownames(TFRC_ery) <- c("MEP", "BFU-E", "CFU-E", "ProE")
TFRC_ery$celltype <- rownames(TFRC_ery)
TFRC_ery$celltype <- factor(
  TFRC_ery$celltype,
  levels = c("MEP", "BFU-E", "CFU-E", "ProE", "Misc")
)

cd71_ery <- ggplot(TFRC_ery, aes(y=2^(avg_log2FC), x=celltype, fill = -log10(p_val_adj))) + 
  geom_col(width = 0.5, color = "black") + 
  geom_hline(yintercept = 0, color = "black", linewidth = 1) + 
  geom_hline(yintercept = 1, color = "black", linetype = "dashed", linewidth = 1) + 
  theme(
  ...
  scale_y_continuous(limits = c(0, 1.05)) + 
  scale_fill_gradient2(low="blue", mid="white", high="red", midpoint = -log10(0.05), limits = c(0, 2.6)) + 
  labs(x="Celltype", y="Fold Change", title="Change in CD71 Expression") 
cd71_ery

ggsave("cd71eryfc.png", cd71_ery, dpi = 600)
