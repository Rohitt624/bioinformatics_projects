...
setwd("C:/Users/rohit/OneDrive - Loyola University Chicago/Zhang Lab/MDS Project/RNA Seq/GSE114922/")
```
...
tbk1plot <- ggplot(df_ordered2, aes(x = reorder(sample, tbk1), y = tbk1, fill = Mutation)) + 
  geom_col() + 
  labs(title = "Caspase 8 Expression in MDS", x = "Sample", y = "Caspase 8 Expression (vst)") + 
  theme_bw() + 
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank())
tbk1plot

ggsave("casp8_patientsamples_vst.png", tbk1plot, dpi = 600)

```

```{r vlnplot}
tbk1plot <- ggplot(df_ordered2, aes(x = factor(Mutation), y = tbk1, fill = Mutation)) + 
  geom_violin(alpha = 0.7, trim = FALSE) + 
  geom_jitter(width = 0.1, alpha = 0.5) + 
  labs(title = "TBK1 Expression in MDS", y = "TBK1 Expression (vst)") + 
  theme_minimal()
tbk1plot

ggsave("tbk1_patientsamples_vln.png", tbk1plot, dpi = 600)

write.csv(df_ordered2, "tak1expression.csv")
```
...
```{r gsea}
run_gsea(dds, "SRSF2", "Healthy", "humangseapathways.gmt", "srsf2gsea.csv", "srsf2deg.csv")

run_gsea(dds, "SRSF2", "Other", "humangseapathways.gmt", "srsf2mdsgsea.csv", "srsf2mdsdeg.csv")
```
...
```{r}
gs <- read.csv("inflammationgseapathways.csv")
gs$pathway <- gsub("_", " ", gs$pathway)
gsplot <- ggplot(gs, aes(x=-log10(padj+0.0000000001), y=reorder(pathway, NES), fill = NES)) +
...
ggsave("gsplot.png", gsplot, dpi = 600, width = 6.5, height = 8)
```

```{r volcano plot}
res_DKO_vs_Tet2 <- results(dds, contrast=c("Mutation", "SF3B1", "Healthy"), alpha = 0.05)
...
deg <- read.csv("selecteddeg.csv")

leading_edge <- bind_rows(top10_up, top10_down, deg)
...
volcano <- ggplot(res_DKO_vs_Tet2, aes(x = log2FoldChange, y = -log10(padj))) +
  ...
  labs(
    title = expression("SF3B1 MDS vs Healthy"),
    x = "log2 Fold Change",
...
ggsave("volcano_sf3b1.png", volcano, dpi = 600)
```