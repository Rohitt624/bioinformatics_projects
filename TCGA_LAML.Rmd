...
setwd("C:/Users/rthalla/OneDrive - Loyola University Chicago/Zhang Lab/TBK1 Project/RNAseq/TCGA-LAML/")
```
...
gsea_cor <- read.csv("gsea_cor_selected.csv")
gsea_plot <- ggplot(gsea_cor, aes(x=NES, y=reorder(pathway, NES), fill = padj)) +
  geom_col(color = "black") + 
  theme(
    panel.background = element_rect(fill = "white"), 
    panel.border = element_rect(color = "black", fill = NA), 
  ...
  labs(x="Normalized Enrichment Score", y="Genesets", title="GSEA Analysis") +
  geom_vline(xintercept = 0, linetype = "solid", color = "black", size = 1)
gsea_plot

gsea_third <- read.csv("gsea_third_selected.csv")
gsea_third_plot <- ggplot(gsea_third, aes(x=NES, y=reorder(pathway, NES), fill = padj)) +
  geom_col(color = "black") + 
  theme(
    panel.background = element_rect(fill = "white"), 
    panel.border = element_rect(color = "black", fill = NA), 
...
ggsave("gsea.png", gsea_plot, width = 10, dpi = 600)
ggsave("gsea_third.png", gsea_third_plot, width = 10, dpi = 600)

```

```{r}
# Convert TBK1 vector into a dataframe
tbk1_df <- data.frame(
  sample_id = names(tbk1_vec),
  tbk1 = as.numeric(tbk1_vec)
) %>%
  dplyr::mutate(
    sample_tcga = gsub("\\.", "-", sample_id),
  ...
  dplyr::select(patient_id, tbk1)

# Merge with clinical table
df_corr <- final_table %>%
  left_join(tbk1_df, by = "patient_id") %>%
  filter(!is.na(tbk1), !is.na(demographic.age_at_index))

cor_test <- cor.test(df_corr$tbk1, df_corr$demographic.age_at_index, method = "spearman")
cor_test

age <- ggplot(df_corr, aes(x = demographic.age_at_index, y = tbk1)) +
  geom_point(alpha = 0.7, color = "steelblue") +
  geom_smooth(method = "lm", se = TRUE, color = "darkred") +
  stat_cor(
    method = "spearman",
    label.x = min(df_corr$demographic.age_at_index, na.rm = TRUE),
    label.y = max(df_corr$tbk1, na.rm = TRUE)
  ) +
  stat_regline_equation(
    label.x = min(df_corr$demographic.age_at_index, na.rm = TRUE),
    label.y = max(df_corr$tbk1, na.rm = TRUE) * 0.9
  ) +
  labs(
    x = "Age",
    y = "TBK1 Expression (normalized counts)",
    title = "Correlation Between TBK1 Expression and Age"
  ) +
  theme_bw()
age

ggsave("tbk1age.png", age, dpi = 600)
