...
setwd("E:/Zhang Lab/CRK HCC/")
```

```{r setup, include=FALSE}
knitr::opts_knit$set(root.dir = 'E:/Zhang Lab/CRK HCC/')
```
...
vln <- ggplot(plot_data_filtered, aes(factor(Subtype), y = KEGG_FOCAL_ADHESION, fill = Subtype)) + 
  geom_violin() + 
...
ggsave("kegg_hoshida_vln_v2.png", vln_ccn2, dpi = 600)
```
...
```{r}
s1_plotdata <- subset(plot_data_filtered, plot_data_filtered$Subtype == "S2")
...
scatter <- ggplot(s1_plotdata, aes(x = KEGG_FOCAL_ADHESION, y = CCN2, color = Subtype)) +
  geom_point(alpha = 0.7, size = 2) +
  geom_smooth(method = "lm", color = "black", se = FALSE, fill = "gray80") +
  labs(x = "KEGG: Focal Adhesion",
    y = "CTGF"
...
ccplot <- ggplot(cc, aes(x=-log10(padj+0.00001), y=reorder(pathway, NES), fill = NES)) +
  ...
  scale_fill_gradient2(low="blue", mid="white", high="red", midpoint=0, limits=c(-2.75,2.75)) + 
  labs(x="-log(padj)", y="Genesets") +
  xlim(0,5.1) + 
  geom_vline(xintercept = -log10(0.25), linetype = "dashed")
ccplot
...
```{r gsva_subtype_scatter}
library(ggplot2)
library(dplyr)

# ── 1. Build combined plot data (all subtypes) ────────────────────────────────
plot_data_all <- data.frame(
  KEGG_FOCAL_ADHESION = as.numeric(vst_extended["KEGG_FOCAL_ADHESION", ]),
  CCN2                = as.numeric(vst_extended["CCN2", ]),
  HIF1A               = as.numeric(vst_extended["HIF1A", ]),
  Subtype             = vst$Hoshida_Subtype
) %>%
  filter(Subtype %in% c("S1", "S2", "S3"))

# ── 2. Pre-compute global lm fits (used as the shared best-fit line) ──────────
lm_ccn2  <- lm(CCN2  ~ KEGG_FOCAL_ADHESION, data = plot_data_all)
lm_hif1a <- lm(HIF1A ~ KEGG_FOCAL_ADHESION, data = plot_data_all)

# ── 3. Helper: Spearman stats label per subtype ───────────────────────────────
spearman_label <- function(df, x_col, y_col) {
  test <- cor.test(df[[x_col]], df[[y_col]], method = "spearman")
  # Return a plain string that ggplot can parse as an expression
  paste0("italic(rho)==", round(test$estimate, 3),
         "~~ italic(P)==", signif(test$p.value, 3))
}

# ── 4. Factory function: one plot per subtype ─────────────────────────────────
make_scatter <- function(subtype, y_col, y_label, color) {

  df_sub <- filter(plot_data_all, Subtype == subtype)

  # Subtype-specific Spearman correlation
  cor_test <- cor.test(df_sub[["KEGG_FOCAL_ADHESION"]], df_sub[[y_col]], method = "spearman")
  lbl <- paste0("italic(rho)==", round(cor_test$estimate, 3),
                "~~ italic(P)==", signif(cor_test$p.value, 3))

  # Subtype-specific fit line
  lm_sub  <- lm(as.formula(paste(y_col, "~ KEGG_FOCAL_ADHESION")), data = df_sub)
  x_seq   <- seq(min(df_sub$KEGG_FOCAL_ADHESION),
                 max(df_sub$KEGG_FOCAL_ADHESION), length.out = 200)
  pred    <- predict(lm_sub,
                     newdata = data.frame(KEGG_FOCAL_ADHESION = x_seq),
                     interval = "confidence")
  line_df <- data.frame(x = x_seq, fit = pred[, "fit"],
                        lwr = pred[, "lwr"], upr = pred[, "upr"])

  ggplot(df_sub, aes(x = KEGG_FOCAL_ADHESION, y = .data[[y_col]])) +
    geom_line(data = line_df,
              aes(x = x, y = fit),
              inherit.aes = FALSE,
              color = "black", linewidth = 0.7, linetype = "dashed") +
    geom_point(color = color, alpha = 0.75, size = 2.5) +
    annotate("text", x = Inf, y = -Inf,
             label = lbl,
             parse = TRUE,
             hjust = 1.1, vjust = -0.5, size = 3.5) +
    labs(title = paste("Hoshida", subtype),
         x = "KEGG: Focal Adhesion (GSVA)",
         y = y_label) +
    theme_bw() +
...
# ── 5. Color palette (match your existing darkred / grey scheme) ──────────────
subtype_colors <- c(S1 = "darkred", S2 = "steelblue", S3 = "darkorange")

# ── 6. Generate all 6 plots ───────────────────────────────────────────────────
plots_ccn2 <- Map(
  function(subtype, color) make_scatter(subtype, "CCN2",  "CTGF (CCN2)", color),
  c("S1", "S2", "S3"),
  unname(subtype_colors[c("S1", "S2", "S3")])
)

plots_hif1a <- Map(
  function(subtype, color) make_scatter(subtype, "HIF1A", "HIF1A", color),
  c("S1", "S2", "S3"),
  unname(subtype_colors[c("S1", "S2", "S3")])
)
...
for (i in seq_along(subtypes)) {
  ggsave(paste0("gsva_ccn2_",  subtypes[i], ".png"),
         plots_ccn2[[i]],  dpi = 600, width = 4, height = 4)
  ggsave(paste0("gsva_hif1a_", subtypes[i], ".png"),
         plots_hif1a[[i]], dpi = 600, width = 4, height = 4)
}


```

```{r fak_subtype_scatter}
library(ggplot2)
library(dplyr)

# ── 1. Build combined plot data (all subtypes) ────────────────────────────────
plot_data_all <- data.frame(
  PTK2    = as.numeric(vst_extended["PTK2", ]),
  CCN2    = as.numeric(vst_extended["CCN2", ]),
  HIF1A   = as.numeric(vst_extended["HIF1A", ]),
  Subtype = vst$Hoshida_Subtype
) %>%
...
# ── 2. Factory function: one plot per subtype ─────────────────────────────────
make_scatter <- function(subtype, y_col, y_label, color) {

  df_sub <- filter(plot_data_all, Subtype == subtype)

  # Subtype-specific Spearman correlation
  cor_test <- cor.test(df_sub[["PTK2"]], df_sub[[y_col]], method = "spearman")
  lbl <- paste0("italic(rho)==", round(cor_test$estimate, 3),
                "~~ italic(P)==", signif(cor_test$p.value, 3))

  # Subtype-specific fit line
  lm_sub  <- lm(as.formula(paste(y_col, "~ PTK2")), data = df_sub)
  x_seq   <- seq(min(df_sub$PTK2), max(df_sub$PTK2), length.out = 200)
  pred    <- predict(lm_sub,
                     newdata = data.frame(PTK2 = x_seq),
                     interval = "confidence")
  line_df <- data.frame(x = x_seq, fit = pred[, "fit"],
                        lwr = pred[, "lwr"], upr = pred[, "upr"])

  ggplot(df_sub, aes(x = PTK2, y = .data[[y_col]])) +
    geom_line(data = line_df,
              aes(x = x, y = fit),
              inherit.aes = FALSE,
              color = "black", linewidth = 0.7, linetype = "dashed") +
...
for (i in seq_along(subtypes)) {
  ggsave(paste0("ptk2_ccn2_",  subtypes[i], ".png"),
         plots_ccn2[[i]],  dpi = 600, width = 4, height = 4)
  ggsave(paste0("ptk2_hif1a_", subtypes[i], ".png"),
         plots_hif1a[[i]], dpi = 600, width = 4, height = 4)
}
```

```{r gsva_combined_scatter}
library(ggforce)
# ── 1. Build plot data ────────────────────────────────────────────────────────
plot_data_all <- data.frame(
  KEGG_FOCAL_ADHESION = as.numeric(vst_extended["KEGG_FOCAL_ADHESION", ]),
...
# ── 2. Combined Spearman label (all subtypes) ─────────────────────────────────
make_combined_label <- function(y_col) {
  cor_test <- cor.test(plot_data_all[["KEGG_FOCAL_ADHESION"]],
                       plot_data_all[[y_col]], method = "spearman")
  paste0("italic(rho)==", round(cor_test$estimate, 3),
         "~~ italic(P)==", signif(cor_test$p.value, 3))
}

# ── 3. Factory function ───────────────────────────────────────────────────────
make_combined_scatter <- function(y_col, y_label) {

  lbl <- make_combined_label(y_col)

  ggplot(plot_data_all, aes(x = KEGG_FOCAL_ADHESION, y = .data[[y_col]])) +
    # Single combined fit line over all data
    geom_smooth(method = "lm", se = FALSE, color = "black", linewidth = 1,
                linetype = "solid") +
    # Points colored by subtype
    geom_point(aes(color = Subtype), alpha = 0.6, size = 2) +
    stat_ellipse(aes(group = Subtype, color = Subtype, fill = Subtype), 
                 geom = "polygon",
                 type = "norm", 
                 linewidth = 1, 
                 level = 0.8,
    ...
    annotate("text", x = -Inf, y = Inf,
             ...
             hjust = -0.05, vjust = 1.5, size = 8) +
    scale_color_manual(values = subtype_colors) +
    scale_fill_manual(values = subtype_colors) + 
    labs(x     = "KEGG: Focal Adhesion (GSVA)",
         y     = y_label,
         color = "Hoshida Subtype") +
    theme_bw(base_size = 18) +
    theme(panel.grid.major = element_line(color = "grey90"),
          panel.grid.minor = element_blank(),
          legend.position  = "bottom")
...
# ── 4. Generate and save ──────────────────────────────────────────────────────
combined_ccn2  <- make_combined_scatter("CCN2",  "CTGF (CCN2)")
combined_hif1a <- make_combined_scatter("HIF1A", "HIF1A")

ggsave("gsva_ccn2_ellipse_filled.png",  combined_ccn2,  dpi = 600, height = 8, width = 8)
ggsave("gsva_hif1a_ellipse_filled.png", combined_hif1a, dpi = 600, height = 8, width = 8)
