...
cell_lines <- grep("MOLM13|MM6|THP1|HL60|U937", colnames(vst), value = TRUE)
vst_subset <- vst[rownames(vst) %in% genelist,
                         colnames(vst) %in% cell_lines]
vst_subset <- as.matrix(vst_subset)
mode(vst_subset) <- "numeric"

# Define color palette
color_palette <- colorRampPalette(c("blue", "white", "red"))(100)

# Create a heatmap
heatmap <- pheatmap(vst_subset, 
         scale = "row", 
         cluster_rows = FALSE, 
         cluster_cols = FALSE, 
         color = color_palette,
