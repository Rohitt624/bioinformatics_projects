...
res_df <- as.data.frame(res_tbk1)

res_df <- data.frame(
  gene = rownames(res_tbk1),
  log2FoldChange = as.numeric(res_tbk1$log2FoldChange),
  padj = as.numeric(res_tbk1$padj),
  pvalue = as.numeric(res_tbk1$pvalue),
  stat = as.numeric(res_tbk1$stat),
...
```{r gsea plot}
inflammation <- read.csv("inflammation_pathways.csv")
infplot <- ggplot(inflammation, aes(x=NES, y=reorder(pathway, NES), fill = padj)) +
  ...
  labs(x="Normalized Enrichment Score", y="Genesets", title="Inflammation Pathways") +
...
autophagy <- read.csv("autophagy_lipid_pathways.csv")
aplplot <- ggplot(autophagy, aes(x=NES, y=reorder(pathway, NES), fill = padj)) +
  geom_col(color = "black") + 
  theme(
    panel.background = element_rect(fill = "white"), 
    panel.border = element_rect(color = "black", fill = NA), 
  ...
  scale_fill_gradient2(low="red", mid="white", high="blue", midpoint= 0.25, limits=c(0, 0.5)) + 
  labs(x="Normalized Enrichment Score", y="Genesets", title="Lipid Metabolism/Autophagy Pathways") +
  geom_vline(xintercept = 0, linetype = "solid", color = "black", size = 1)
aplplot

nfkb <- read.csv("nfkb_stemness_pathways.csv")
nfkbplot <- ggplot(nfkb, aes(x=NES, y=reorder(pathway, NES), fill = padj)) +
  geom_col(color = "black") + 
  theme(
    panel.background = element_rect(fill = "white"), 
    panel.border = element_rect(color = "black", fill = NA), 
  ...
  scale_fill_gradient2(low="red", mid="white", high="blue", midpoint= 0.25, limits=c(0, 0.5)) + 
  labs(x="Normalized Enrichment Score", y="Genesets", title="NF-κB/Stemness Pathways") +
  geom_vline(xintercept = 0, linetype = "solid", color = "black", size = 1)
nfkbplot

ggsave("inflammationgsea2.png", infplot, dpi = 600)
ggsave("celldeathgsea2.png", cdplot, dpi = 600)
ggsave("mitochondriagsea2.png", aplplot, dpi = 600)
ggsave("nfkbgsea2.png", nfkbplot, dpi = 600)
...
# Extract TBK1 vector
tbk1_vec <- as.numeric(norm["TBK1", ])

# Compute correlation for every gene
cor_df <- apply(norm, 1, function(gene_expr) {
  cor(gene_expr, tbk1_vec, method = "spearman", use = "pairwise.complete.obs")
})
...
cor_df <- cor_df %>% arrange(desc(abs(spearman_cor)))
...
df_scatter <- data.frame(
  sample = colnames(norm_counts),
  TBK1 = as.numeric(norm_counts["TBK1", ]),
  RIPK1 = as.numeric(norm_counts["RIPK1", ])
)

fit <- lm(RIPK1 ~ TBK1, data = df_scatter)
...
scatter <- ggplot(df_scatter, aes(x = TBK1, y = RIPK1)) +
  geom_point(size = 3, color = "steelblue") +
  geom_abline(intercept = intercept, slope = slope, color = "darkred", size = 1) +
  annotate("text", 
           x = min(df_scatter$TBK1), 
           y = max(df_scatter$RIPK1), 
           label = label_text, 
           hjust = 0, vjust = 1, size = 5) +
  labs(
    title = "TBK1 vs RIPK1 Expression (Patient Samples)",
    x = "TBK1 Expression (VST)",
    y = "RIPK1 Expression (VST)"
  ) +
  theme_bw() +
  theme(
    plot.title = element_text(size = 14, face = "bold"),
...
ggsave("tbk1ripk1scatter.png", scatter, dpi = 600)
