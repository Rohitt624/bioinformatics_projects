...
subset_rlog <- assay(rlog)[rownames(rlog) %in% genelist, ]

conditions <- c("Y2P", "YMP", "Y2N", "YMN")
cdsub <- cd[cd$condition %in% conditions, ]
cdsub$condition <- factor(cdsub$condition, levels = c("Y2P", "YMP", "Y2N", "YMN"))
...
rloggenes <- rloggenes[,order(cdsub$X)]
...
heatmap <- pheatmap(rloggenes, 
         ...
         cellheight = 10)
heatmap
ggsave("pnheatmap5.png", heatmap, height = 15, width = 10, dpi = 600)
