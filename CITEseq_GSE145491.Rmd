...
Idents(seurat_object) <- "gates"
adtscatter <- FeatureScatter(seurat_object, feature1 = "adt_CD150", feature2 = "adt_CD135") +
                scale_x_log10() + 
                scale_y_log10() + 
                scale_color_manual(values = c("MPP4" = "red", "CD135-" = "blue", "NA" = "grey"))
adtscatter
...
threshold <- quantile(gating_value$adt_CD135, 0.55)

seurat_object$gates <- ifelse(gating$adt_CD135 > threshold & gating$adt_CD150 < 200 & gating$adt_CD150 > 0, "MPP4", ifelse(gating$adt_CD150 > 0 & gating$adt_CD135 > 0, "CD135-", NA))
...
Idents(cd135neg) <- "gates2"
gating <- FetchData(cd135neg, vars = c("adt_CD150", "adt_CD48"))
compress_y <- trans_new(
  name = "compress_log",
  transform = function(y) {
    ifelse(y <= 1000, 0.2333 * log10(y), 0.7 * (log10(y) - 3) / (4 - 3) + 0.7)
  },
  inverse = function(y) {
    ifelse(y <= 0.3, 10^(y / 0.3), 10^(3 + (y - 0.3) * (4 - 3) / 0.7))
  }
)
...
```{r}
y_values <- seq(1, 10000, length.out = 1000)
transformed_y <- compress_y$transform(y_values)

ggplot(data.frame(y = y_values, transformed_y = transformed_y), aes(x = y, y = transformed_y)) +
  geom_line() +
  scale_x_log10() +
  labs(title = "Transformation Function", x = "Original y", y = "Transformed y")
```
...
Verify
``` {r}
table(seurat_object$gates)
table(cd135neg$gates2)
```

```{r subset mpp3}
mpp3 <- subset(cd135neg, idents = "MPP3")
```
...
mpp3$ESAM <- "ESAMmid"
mpp3$ESAM[esam > seventy] <- "ESAM+"
mpp3$ESAM[esam < thirty & esam > 0] <- "ESAM-"

```

```{r ESAM featurescatter}
Idents(mpp3) <- "ESAM"
adtscatter <- FeatureScatter(mpp3, feature1 = "adt_CD150", feature2 = "adt_ESAM") +
                scale_x_log10() + 
                scale_y_log10()
...
```{r ESAM DEG GSEA}
Idents(mpp3) <- "ESAM"
adt_markers <- FindMarkers(mpp3, ident.1 = "ESAM+", ident.2 = "ESAM-", assay = "ADT")
rna_markers <- FindMarkers(mpp3, ident.1 = "ESAM+", ident.2 = "ESAM-", assay = "RNA")

write.csv(adt_markers, "Output/adtmpp3.csv")
write.csv(rna_markers, "Output/rnampp3.csv")

EnsIDReplace2(rna_markers, mpp3esam)
write.csv(rna_markers, "Output/rnampp3_genename.csv")

run_gsea(rna_markers, "reactome_go_gsea.gmt", "Output/mpp3esam_gsea.csv")
```
