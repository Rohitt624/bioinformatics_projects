...
cd135neg$gate3 <- NA
cd135neg$gate3 <- ifelse(cd135work$adt_CD48 > 5000 & cd135work$adt_CD150 < 270, "MPP3", NA)

top40 <- quantile(mpp3threshold$adt_ESAM, 0.6)
bottom40 <- quantile(mpp3threshold$adt_ESAM, 0.4)
mpp3$gate4 <- NA
mpp3$gate4 <- ifelse(mpp3threshold$adt_ESAM > top40, "ESAMhigh",
                     ifelse(mpp3threshold$adt_ESAM < bottom40, "ESAMlow", "ESAMmid"))
Idents(mpp3) <- "gate4"
...
DimPlot(mpp3, group.by = "seurat_clusters")
FeaturePlot(mpp3, features = c("rna_ENSMUSG00000001946"))
...
```{r protein-v-rna}
FeatureScatter(filter, feature1 = "adt_ESAM", feature2 = "rna_ENSMUSG00000001946", log=TRUE) + labs(title="ESAM RNA vs Protein")+ 
...
filter <- subset(seurat_object, cells = rownames(esamdata))
