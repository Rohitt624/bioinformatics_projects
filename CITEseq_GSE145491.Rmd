...
setwd("E:/Zhang Lab/Archives/RNASeq/RNASeq/RNASeq/Cite-seq/GSE145491")
#setwd("/")
```
...
run_gsea_seurat_all <- function(all_markers_df, gmt_path, output_dir, output_deg_all,
                                  logfc_col = "avg_log2FC", padj_col = "p_val_adj",
  ...
  req_cols <- c(logfc_col, padj_col, "cluster")
  if (!all(req_cols %in% colnames(all_markers_df))) {
    stop(paste("Input must be a FindAllMarkers output containing:", 
               paste(req_cols, collapse = ", ")))
  }

  all_markers_df$cluster <- as.character(all_markers_df$cluster)

  # Overwrite gene column with de-suffixed rownames (restores original, possibly
  # non-unique gene IDs collapsed by FindAllMarkers' automatic .1/.2 renaming)
  all_markers_df$gene <- sub("\\.\\d+$", "", rownames(all_markers_df))

  if (!dir.exists(output_dir)) dir.create(output_dir, recursive = TRUE)

  write.csv(all_markers_df, output_deg_all, row.names = FALSE)

  gmt <- fgsea::gmtPathways(gmt_path)

  clusters <- unique(all_markers_df$cluster)
  gsea_results_list <- list()
  ranking_list <- list()

  for (clust in clusters) {

    message("Running GSEA for cluster: ", clust)

    keep_rows <- all_markers_df$cluster == clust & !is.na(all_markers_df[[logfc_col]])
    sub_df <- all_markers_df[keep_rows, , drop = FALSE]

    if (nrow(sub_df) == 0) {
      warning(paste("No valid markers for cluster", clust, "- skipping"))
      next
    }

    # Deduplicate by gene, keeping strongest |log2FC| (now using the restored gene column)
    sub_df <- sub_df[order(-abs(sub_df[[logfc_col]])), , drop = FALSE]
    sub_df <- sub_df[!duplicated(sub_df$gene), , drop = FALSE]

    rnk <- setNames(sub_df[[logfc_col]], sub_df$gene)
    rnk <- sort(rnk, decreasing = TRUE)

    if (length(rnk) < 1000) {
      warning(paste0("Cluster '", clust, "': ranking vector is small (<1000 genes). ",
                      "fgsea may lack statistical power. Ensure logfc.threshold = 0 in FindAllMarkers."))
    }

    ranking_list[[as.character(clust)]] <- rnk

    gsea <- tryCatch({
      fgsea(pathways = gmt, stats = rnk, minSize = 10, maxSize = 500)
    }, error = function(e) {
      warning(paste0("fgsea failed for cluster '", clust, "': ", e$message))
      return(NULL)
    })

    if (is.null(gsea) || nrow(gsea) == 0) {
      warning(paste0("No pathways passed size filters for cluster '", clust, 
                      "' - check gene ID overlap with GMT. Skipping."))
      next
    }
    ...
    gsea_results_list[[as.character(clust)]] <- gsea_df

    safe_clust_name <- gsub("[^A-Za-z0-9_-]", "_", as.character(clust))
    out_path <- file.path(output_dir, paste0("gsea_", safe_clust_name, ".csv"))
    write.csv(gsea_df, out_path, row.names = FALSE)
  }

  combined_gsea <- do.call(rbind, gsea_results_list)
  write.csv(combined_gsea, file.path(output_dir, "gsea_combined_all_clusters.csv"), row.names = FALSE)

  sig_gsea <- combined_gsea[combined_gsea$padj < gsea_padj_cutoff & !is.na(combined_gsea$padj), , drop = FALSE]

  overlap_matrix <- sig_gsea %>%
    dplyr::select(pathway, cluster, NES, padj) %>%
    tidyr::pivot_wider(names_from = cluster, values_from = c(NES, padj))

  overlap_summary <- sig_gsea %>%
    dplyr::group_by(pathway) %>%
    dplyr::summarise(
      n_clusters_significant = dplyr::n_distinct(cluster),
      clusters = paste(sort(unique(cluster)), collapse = ";"),
      mean_NES = mean(NES),
      direction_consistent = length(unique(sign(NES))) == 1,
      .groups = "drop"
    ) %>%
    dplyr::arrange(desc(n_clusters_significant), desc(abs(mean_NES)))

  write.csv(overlap_matrix, file.path(output_dir, "gsea_overlap_matrix.csv"), row.names = FALSE)
  write.csv(overlap_summary, file.path(output_dir, "gsea_overlap_summary.csv"), row.names = FALSE)

  unique_pathways <- overlap_summary[overlap_summary$n_clusters_significant == 1, , drop = FALSE]
  names(unique_pathways)[names(unique_pathways) == "clusters"] <- "cluster"

  write.csv(unique_pathways, file.path(output_dir, "gsea_unique_by_cluster.csv"), row.names = FALSE)

  invisible(list(
    gsea_by_cluster = gsea_results_list,
    combined_gsea = combined_gsea,
    overlap_summary = overlap_summary,
    overlap_matrix = overlap_matrix,
    unique_pathways = unique_pathways,
    rankings_by_cluster = ranking_list,
...
filter$gate2 <- NA
filter$gate2 <- ifelse(cd135$adt_CD135 > 250 & cd135$adt_CD150 < 270, "MPP4", 
                       ifelse(cd135$adt_CD135 < 250, "CD135-", NA))
cd135neg <- subset(filter, gate2 == "CD135-")

rm(cd135)
adtscatter <- FeatureScatter(filter, feature1 = "adt_CD150", feature2 = "adt_CD135", group.by = "gate2") + 
...
FeatureScatter(cd135neg, feature1 = "adt_CD150", feature2 = "adt_CD48", group.by = "gate3") + 
  scale_x_flowjo_biexp(pos = 5) + 
  scale_y_flowjo_biexp(widthBasis = -150,
                       pos = 3)
```

```{r add gates back to original filtered seurat object}
filter <- subset(filter, adt_CD150>0 & adt_CD135>0 & adt_CD48>0)
# start with NA for every cell in the full object
final_gates <- rep(NA_character_, ncol(filter))
names(final_gates) <- colnames(filter)

# MPP4 cells came from filter$gate2 (before you subsetted to cd135neg)
mpp4_cells <- colnames(filter)[!is.na(filter$gate2) & filter$gate2 == "MPP4"]
final_gates[mpp4_cells] <- "MPP4"

# MPP1/MPP2/MPP3/HSC cells came from cd135neg$gate3
gate3_named <- setNames(cd135neg$gate3, colnames(cd135neg))
gate3_named <- gate3_named[!is.na(gate3_named)]
final_gates[names(gate3_named)] <- gate3_named

# attach back to the original object, matched by cell name
filter <- AddMetaData(filter, metadata = final_gates, col.name = "gating")

# sanity check
table(filter$gating, useNA = "ifany")
```

```{r just filter}
rm(cd135neg, cd135work, seurat_object)
```

```{r TBK1 + OPTN FeaturePlots}
adtscatter <- FeatureScatter(filter, feature1 = "adt_CD150", feature2 = "adt_CD48", group.by = "gating") + 
  scale_x_flowjo_biexp(pos = 5) + 
  scale_y_flowjo_biexp(widthBasis = -150,
                       pos = 3)+ 
  geom_density_2d(bins = 15) + 
  geom_hline(yintercept = 5000, linetype = "solid", color = "black", linewidth = 1) +
  geom_vline(xintercept = 270, linetype = "solid", color = "black", linewidth = 1) + 
  labs(title = "LSKs CD48 vs CD150", x = "CD150", y = "CD48")
adtscatter
ggsave("TBK1 work/LSKgating.png", adtscatter, dpi = 600)


#Fetch ESAM (ADT), CD27 (RNA), and Fcgr3 (RNA)
plot_df <- FetchData(filter, vars = c("adt_CD48", 
                                              "adt_CD150",   
                                              "rna_ENSMUSG00000020115", #TBK1
                                             "rna_ENSMUSG00000026672" #OPTN
                                             ))  

# Sort so low Fcgr3 cells plot first (high expression on top)
plot_df <- plot_df[order(plot_df$rna_ENSMUSG00000020115), ]

# Plot
tbk1scatter <- ggplot(plot_df, aes(x = adt_CD150, 
                                     y = adt_CD48, 
                                     color = rna_ENSMUSG00000020115)) +
  geom_point(size = 0.5, alpha = 0.7) +
  scale_color_gradientn(
    colors = c("lightgrey", "blue", "darkblue"),
    name = "TBK1"
  ) +
  scale_x_flowjo_biexp(pos = 5) +
  scale_y_flowjo_biexp(pos = 3, widthBasis = -150) +
  geom_hline(yintercept = 5000, linetype = "solid", color = "black", linewidth = 1) +
  geom_vline(xintercept = 270, linetype = "solid", color = "black", linewidth = 1) +
  labs(title = "TBK1",
       x = "CD150", 
...
tbk1scatter
ggsave("TBK1 work/tbk1scatter.png", tbk1scatter, dpi = 600)

plot_df <- plot_df[order(plot_df$rna_ENSMUSG00000026672), ]

optnscatter <- ggplot(plot_df, aes(x = adt_CD150, 
                                     y = adt_CD48, 
                                     color = rna_ENSMUSG00000026672)) +
  geom_point(size = 0.5, alpha = 0.7) +
  scale_color_gradientn(
    colors = c("lightgrey", "blue", "darkblue"),
    values = scales::rescale(c(0, 0.22, 1)),
    limits = c(0, 1),
    name = "OPTN"
  ) +
  scale_x_flowjo_biexp(pos = 5) +
...
optnscatter
ggsave("TBK1 work/optnscatter.png", optnscatter, dpi = 600)
```

```{r TBK1 + OPTN expression data}
tbk1vln <- VlnPlot(filter, features = c("rna_ENSMUSG00000020115"), group.by = "gating")
tbk1vln
optnvln <- VlnPlot(filter, features = ("rna_ENSMUSG00000026672"), group.by = "gating")
optnvln

plot_df <- FetchData(filter, vars = c("rna_ENSMUSG00000020115", #TBK1
                                      "rna_ENSMUSG00000026672", #OPTN
                                      "gating"
                                             ))
mean <- AverageExpression(filter, 
                          features = c("ENSMUSG00000020115", "ENSMUSG00000026672"), 
                          group.by = "gating")
tbk1bar <- ggplot(mean, aes(x=gating, y=ENSMUSG00000020115, fill=gating)) + 
  geom_bar(color="black") + 
  labs(title="Average TBK1 Expression in LSKs", y="TBK1 expression")
tbk1bar

genes <- c("ENSMUSG00000020115", "ENSMUSG00000026672", "ENSMUSG00000028756", "ENSMUSG00000023826", "ENSMUSG00000015837")
gene_labels <- c("ENSMUSG00000020115" = "TBK1", 
                  "ENSMUSG00000026672" = "OPTN",
                 "ENSMUSG00000028756" = "PINK1",
                 "ENSMUSG00000023826" = "PARKIN",
                 "ENSMUSG00000015837" = "P62")

expr_data <- FetchData(filter, vars = c(genes, "gating")) %>%
  dplyr::filter(!is.na(gating), gating != "NA")

summary_df <- expr_data %>%
  pivot_longer(cols = all_of(genes), names_to = "gene", values_to = "expression") %>%
  group_by(gene, gating) %>%
  summarise(
    mean_expr = mean(expression),
    sd_expr = sd(expression),
    sem_expr = sd(expression) / sqrt(n()),
    n = n(),
    .groups = "drop"
  ) %>%
  mutate(gene_label = recode(gene, !!!gene_labels))

# TBK1 plot
tbk1_df <- summary_df %>% dplyr::filter(gene_label == "TBK1")

p_tbk1 <- ggplot(tbk1_df, aes(x = gating, y = mean_expr, fill = gating)) +
  geom_bar(stat = "identity", color = "black", width = 0.7) +
  geom_errorbar(aes(ymin = mean_expr - sem_expr, ymax = mean_expr + sem_expr), width = 0.2) +
  geom_text(aes(y = mean_expr + sem_expr, label = paste0("n=", n)), 
            vjust = -0.5, size = 3) +
  labs(title = "TBK1", x = "Identity", y = "Average Expression Level") +
  theme_classic() +
  theme(legend.position = "none",
        axis.text.x = element_text(angle = 45, hjust = 1))

# OPTN plot
optn_df <- summary_df %>% dplyr::filter(gene_label == "OPTN")

p_optn <- ggplot(optn_df, aes(x = gating, y = mean_expr, fill = gating)) +
  geom_bar(stat = "identity", color = "black", width = 0.7) +
  geom_errorbar(aes(ymin = mean_expr - sem_expr, ymax = mean_expr + sem_expr), width = 0.2) +
  geom_text(aes(y = mean_expr + sem_expr, label = paste0("n=", n)), 
            vjust = -0.5, size = 3) +
  labs(title = "OPTN", x = "Identity", y = "Average Expression Level") +
  theme_classic() +
  theme(legend.position = "none",
        axis.text.x = element_text(angle = 45, hjust = 1))
...
write.csv(summary_df, "TBK1 work/geneexpression.csv")

```

```{r gsea}
markers <- FindAllMarkers(filter, group.by = "gating")
markers <- EnsIDReplace2(markers)
run_gsea_seurat_all(all_markers_df = markers,
                    gmt_path = "mouse_autophagypathways_gsea.gmt",
                    output_dir = "TBK1 work",
                    output_deg_all = "lskmarkers.csv")
```
...
VlnPlot(mpp3, features = c("rna_ENSMUSG00000030336"))
...
```{r CD16/32 FeaturePlot}
# Fetch ESAM (ADT), CD27 (RNA), and Fcgr3 (RNA)
plot_df <- FetchData(mpp3_filtered, vars = c("adt_ESAM", 
                                              "rna_ENSMUSG00000030336",   # CD27
                                              "rna_ENSMUSG00000022021"))  # Fcgr3 — verify this ID

# Sort so low Fcgr3 cells plot first (high expression on top)
plot_df <- plot_df[order(plot_df$rna_ENSMUSG00000022021), ]

# Plot
fcgr3scatter <- ggplot(plot_df, aes(x = adt_ESAM, 
                                     y = rna_ENSMUSG00000030336, 
                                     color = rna_ENSMUSG00000022021)) +
  geom_point(size = 0.5, alpha = 0.7) +
  scale_color_gradientn(
    colors = c("lightgrey", "blue", "darkblue"),
    name = "Diaph3 (RNA)"
  ...
  scale_x_flowjo_biexp(pos = 5) +
  scale_y_flowjo_biexp(pos = 8, neg = 2, widthBasis = -500) +
  geom_hline(yintercept = cd27_cutoff, linetype = "solid", color = "black", linewidth = 1) +
  geom_vline(xintercept = esam_cutoff, linetype = "solid", color = "black", linewidth = 1) +
  labs(title = "Diaph3",
       x = "ESAM (ADT)", 
...
fcgr3scatter
ggsave("mpp3esamcd27_diaph3.png", fcgr3scatter, dpi = 600)
```

Gp9 = rna_ENSMUSG00000030054
Gata2 = rna_ENSMUSG00000015053
Diaph3 = rna_ENSMUSG00000022021

```{r}
install.packages("https://cran.r-project.org/src/contrib/Archive/grr/grr_0.9.5.tar.gz", 
                 repos = NULL, 
                 type = "source")
remotes::install('cole-trapnell-lab/monocle3')
...
# --- Step 1: Convert to Monocle3 CDS ---
cds <- as.cell_data_set(mpp3_filtered)
cds <- cluster_cells(cds)
cds <- learn_graph(cds)

# --- Step 2: Set DP cells as the root ---
# Get cell names of your DP population
dp_cells <- rownames(mpp3_filtered@meta.data[mpp3_filtered$gate5 == "DP", ])

# Order cells with DP as root
cds <- order_cells(cds, root_cells = dp_cells)

# --- Step 3: Pull pseudotime back into Seurat ---
mpp3_filtered$pseudotime <- pseudotime(cds)

# --- Step 4: Build your scatter plot colored by pseudotime ---
plot_df <- FetchData(mpp3_filtered, vars = c("adt_ESAM", 
                                              "rna_ENSMUSG00000030336"))  # CD27

plot_df$pseudotime <- mpp3_filtered$pseudotime

# Sort so early pseudotime plots on top
plot_df <- plot_df[order(-plot_df$pseudotime), ]
# Option 2: If still compressed, try log-scaling pseudotime
plot_df$pseudotime_log <- log1p(plot_df$pseudotime_clipped)  # log1p handles zeros safely

pseudotime_scatter_log <- ggplot(plot_df, aes(x = adt_ESAM, 
                                               y = rna_ENSMUSG00000030336, 
                                               color = pseudotime_log)) +
  geom_point(size = 0.5, alpha = 0.7) +
  scale_color_gradientn(
    colors = c("blue", "cyan", "green3", "yellow"),
    na.value = "lightgrey",
    name = "Pseudotime\n(log)"
  ) +
  scale_x_flowjo_biexp(pos = 5) +
  ...
  geom_hline(yintercept = cd27_cutoff, linetype = "solid", color = "black", linewidth = 1) +
  geom_vline(xintercept = esam_cutoff, linetype = "solid", color = "black", linewidth = 1) +
  labs(title = "MPP3 ESAM vs CD27 — Pseudotime from DP",
       x = "ESAM (ADT)", 
       y = "CD27 (RNA)") +
  theme_classic()
pseudotime_scatter_log

ggsave("mpp3esamcd27_pseudotime.png", pseudotime_scatter, dpi = 600)

# Check your pseudotime distribution first
summary(plot_df$pseudotime[is.finite(plot_df$pseudotime)])
quantile(plot_df$pseudotime[is.finite(plot_df$pseudotime)], c(0.25, 0.5, 0.75, 0.9, 0.95))

```

```{r geneset creation}
Idents(mpp3_filtered) <- "gate5"
genesets <- FindAllMarkers(mpp3_filtered, only.pos = TRUE, min.pct = 0.05)
genesets <- EnsIDReplace2(genesets)
write.csv(genesets, "mpp3genesetsv2.csv")
```

```{r gsea of DP}
markers_DP <- FindMarkers(mpp3_filtered, 
                          ident.1 = "DP", 
                          ident.2 = c("DN", "27+", "E+"),
                          logfc.threshold = 0,   # don't filter by FC
                          min.pct = 0)           # don't filter by expression

markers_DP <- EnsIDReplace2(markers_DP)
run_gsea_seurat(markers_DP, 
                "C:/Users/rthalla/OneDrive - Loyola University Chicago/Zhang Lab/reactome_go_gsea.gmt",
                "dp_gsea.csv",
                "dp_deg.csv")
write.csv(markers_DP, "dp_deg.csv")
```

```{r gsea of DN}
markers_DN <- FindMarkers(mpp3_filtered, 
                          ident.1 = "DN", 
                          ident.2 = c("DP", "27+", "E+"),
                          logfc.threshold = 0,   # don't filter by FC
                          min.pct = 0)           # don't filter by expression

markers_DN <- EnsIDReplace2(markers_DN)
run_gsea_seurat(markers_DN, 
                "C:/Users/rthalla/OneDrive - Loyola University Chicago/Zhang Lab/reactome_go_gsea.gmt",
                "DN_gsea.csv",
                "DN_deg.csv")
write.csv(markers_DN, "DN_deg.csv")
```

```{r gsea of E+}
markers_E <- FindMarkers(mpp3_filtered, 
                          ident.1 = "E+", 
                          ident.2 = c("DN", "27+", "DP"),
                          logfc.threshold = 0,   # don't filter by FC
                          min.pct = 0)           # don't filter by expression

markers_E <- EnsIDReplace2(markers_E)
run_gsea_seurat(markers_E, 
                "C:/Users/rthalla/OneDrive - Loyola University Chicago/Zhang Lab/reactome_go_gsea.gmt",
                "E+_gsea.csv",
                "E+_deg.csv")
write.csv(markers_E, "E+_deg.csv")
```

```{r gsea of 27+}
markers_27 <- FindMarkers(mpp3_filtered, 
                          ident.1 = "27+", 
                          ident.2 = c("DN", "DP", "E+"),
                          logfc.threshold = 0,   # don't filter by FC
                          min.pct = 0)           # don't filter by expression

markers_27 <- EnsIDReplace2(markers_27)
run_gsea_seurat(markers_27, 
                "C:/Users/rthalla/OneDrive - Loyola University Chicago/Zhang Lab/reactome_go_gsea.gmt",
                "27+_gsea.csv",
                "27+_deg.csv")
write.csv(markers_27, "27_deg.csv")
```

```{r myeloid gsea plot}
table <- read.csv("myeloid_immunity.csv")
gseaplot <- ggplot(table, aes(x=-log10(padj), y=factor(pathway, levels = c("DN", "27+", "DP", "E+")), fill = NES, size = size)) +
  geom_point(shape=21) + 
  ...
  scale_fill_gradient2(low="blue", mid="purple", high="red", midpoint=0, limits=c(-2.3,2.3)) + 
  labs(x="-log(padj)", y="Population", title = "GOBP MYELOID LEUKOCYTE MEDIATED IMMUNITY
") +
  xlim(0,4) + 
  geom_vline(xintercept = -log10(0.25), linetype = "dashed")
gseaplot

ggsave("myeloidv3.png", gseaplot, dpi = 600, height = 8, width = 6)
```

```{r DC gsea plot}
table <- read.csv("dendriticcell.csv")
gseaplot <- ggplot(table, aes(x=-log10(padj), y=factor(pathway, levels = c("DN", "27+", "DP", "E+")), fill = NES, size = size)) +
  geom_point(shape=21) + 
  scale_size(range = c(5, 10)) +
  theme(
  ...
  scale_fill_gradient2(low="blue", mid="purple", high="red", midpoint=0, limits=c(-2.3,2.3)) + 
  labs(x="-log(padj)", y="Population", title = "GOBP MYELOID DENDRITIC CELL DIFFERENTIATION") +
  xlim(0,4) + 
  geom_vline(xintercept = -log10(0.25), linetype = "dashed")
gseaplot

ggsave("dcv3.png", gseaplot, dpi = 600, height = 8, width = 6)
```

```{r platelet gsea plot}
table <- read.csv("platelet.csv")
gseaplot <- ggplot(table, aes(x=-log10(padj), y=factor(pathway, levels = c("DN", "27+", "DP", "E+")), fill = NES, size = size)) +
  geom_point(shape=21) + 
  scale_size(range = c(5, 10)) +
  theme(
  ...
  scale_fill_gradient2(low="blue", mid="purple", high="red", midpoint=0, limits=c(-2.3,2.3)) + 
  labs(x="-log(padj)", y="Population", title = "GOBP PLATELET ACTIVATION") +
  xlim(0,2) + 
  geom_vline(xintercept = -log10(0.25), linetype = "dashed")
gseaplot

ggsave("plateletv3.png", gseaplot, dpi = 600, height = 8, width = 6)
```

```{r CC gsea plot}
table <- read.csv("cellcycle.csv")
gseaplot <- ggplot(table, aes(x=-log10(padj), 
                              y=factor(pathway, levels = c("DN", "27+", "DP", "E+")),
                              fill = NES, 
                              size = size)) +
  geom_point(shape=21) + 
  scale_size(range = c(5, 10)) +
  ...
  scale_fill_gradient2(low="blue", mid="purple", high="red", midpoint=0, limits=c(-2.3,2.3)) + 
  labs(x="-log(padj)", y="Population", title = "REACTOME CELL CYCLE CHECKPOINTS") +
  xlim(0,10) + 
  geom_vline(xintercept = -log10(0.25), linetype = "dashed")
gseaplot

ggsave("cellcyclev3.png", gseaplot, dpi = 600, height = 8, width = 6)
```

```{r Erythroid gsea plot}
table <- read.csv("erythroid.csv")
gseaplot <- ggplot(table, aes(x=-log10(padj), y=factor(pathway, levels = c("DN", "27+", "DP", "E+")), fill = NES, size = size)) +
  geom_point(shape=21) + 
  scale_size(range = c(5, 10)) +
  theme(
  ...
  scale_fill_gradient2(low="blue", mid="purple", high="red", midpoint=0, limits=c(-2,2)) + 
  labs(x="-log(padj)", y="Population", title = "GOBP ENUCLEATE ERYTHROCYTE DIFFERENTIATION") +
  xlim(0,2) + 
  geom_vline(xintercept = -log10(0.25), linetype = "dashed")
gseaplot


ggsave("erythroidv3.png", gseaplot, dpi = 600, height = 8, width = 6)
```

```{r HSC gsea plot}
table <- read.csv("stemcell.csv")
gseaplot <- ggplot(table, aes(x=-log10(padj), 
                              y=factor(pathway, levels = c("DN", "27+", "DP", "E+")), 
                              fill = NES, 
                              size = size)) +
  ...
  scale_fill_gradient2(low="blue", mid="purple", high="red", midpoint=0, limits=c(-2.3,2.3)) + 
  labs(x="-log(padj)", y="Population", title = "GOBP HEMATOPOIETIC STEM CELL HOMEOSTASIS") +
  xlim(0,2) + 
  geom_vline(xintercept = -log10(0.25), linetype = "dashed")
gseaplot

ggsave("stemcellv3.png", gseaplot, dpi = 600, height = 8, width = 6)
```
