...
setwd("E:/Zhang Lab/Archives/RNASeq/RNASeq/Cite-seq/GSE145491")
setwd("/")
```
...
```{r}
filter <- subset(filter, adt_CD150<0 & act_CD135>0 & adt_CD48>0)
...
```{r}
markers <- FindAllMarkers(seurat_object, group.by = "gating")

tbk1scatter <- FeatureScatter(filter, feature1 = "adt_CD150", feature2 = "adt_CD48") + 
  scale_x_flowjo_biexp(pos = 5) + 
  scale_y_flowjo_biexp(widthBasis = -150,
                       pos = 3)+ 
...
plot_df <- FetchData(mpp3_filtered, vars = c("adt_CD48", 
                                              "adt_CD150",   # CD27
                                              "rna_ENSMUSG00000020115", #TBK1
                                             "rna_ENMUSG00000026672" #OPTN
                                             ))  
...
tbk1scatter
ggsave("tbk1scatter.png", tbk1scatter, dpi = 600)
...
tbk1scatter
ggsave("optnscatter.png", optnscatter, dpi = 600)
```

```{r}
tbk1vln <- VlnPlot(filter, features = c("rna_ENSMUSG00000020115"))
tbk1vln
optnvln <- VlnPlot(filter, features = ("rna_ENSMUSG00000026672"))
optnvln
```
