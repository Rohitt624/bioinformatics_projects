...
```{r featurescatter}
FeatureScatter(seurat_object, feature1 = "adt_CD150", feature2 = "adt_ESAM", log = TRUE, group.by = "gating")
```
...
mpp3 <- subset(seurat_object, idents = "CD135- MPP3")
```

```{r more gating, eval=FALSE}
subsetmpp3 <- FeatureScatter(mpp3, feature1 = "adt_CD150", feature2 = "adt_ESAM", log = TRUE)
mpp3 <- CellSelector(subsetmpp3, object = mpp3, ident = "ESAM-")

```
Save gating
```{r save gating, eval=FALSE}
active_idents <- Idents(mpp2)
gates <- c("ESAM+", "ESAM-")
mpp2$gate <- ifelse(active_idents %in% gates, as.character(active_idents), NA)
```

```{r, eval=FALSE}
seurat_object$gating[Cells(mpp2)] <- paste("CD135- MPP2", mpp2$gate)
DimPlot(seurat_object, group.by = "gating")
```
...
```{r vlnplot}
VlnPlot(seurat_object, features = MegcombEns)
```

### View relationship between protein and RNA
```{r protein-v-rna}
FeatureScatter(seurat_object, feature1 = "adt_ESAM", feature2 = "rna_ENSMUSG00000052301")
```
...
MegcombEns <- gene_ids[["ensembl_gene_id"]]
...
```{r}
DefaultAssay(seurat_object) <- "RNA"
DoHeatmap(seurat_object, features = MegcombEns)
```

```{r}
expression_data <- FetchData(seurat_object, vars = MegcombEns)
expression_data$gating <- seurat_object@meta.data$gating

# Filter data to include only the desired groups
desired_groups <- c("CD135- HSC ESAM+", "CD135- MPP1 ESAM+", "CD135- MPP2 ESAM+", "CD135- MPP3 ESAM+")
filtered_data <- expression_data[expression_data$gating %in% desired_groups, ]

# Prepare heatmap data
heatmap_data <- as.matrix(filtered_data[, MegcombEns])
