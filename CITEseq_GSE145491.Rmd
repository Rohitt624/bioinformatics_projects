...
FeatureScatter(seurat_object, feature1 = "adt_CD150", feature2 = "adt_CD135", log = TRUE)
...
test <- subset(seurat_object, idents = "MPP4")
```
limits = c(3, 500)
limits = c(20, NA)
...
gating_value <- gating_value[gating_value$adt_CD150 < 100,]
threshold <- quantile(gating_value$adt_CD135, 0.5)

seurat_object$gates <- ifelse(gating$adt_CD135 > threshold & gating$adt_CD150 < 100 & gating$adt_CD150 > 0, "MPP4", ifelse(gating$adt_CD135 < threshold & gating$adt_CD150 > 0 & gating$adt_CD135 > 0, "CD135-", NA))
seurat_object$gates <- ifelse(gating$adt_CD135 < threshold & gating$adt_CD150 > 0 & gating$adt_CD135 > 0, "CD135-", NA)
seurat_object$ESAM[esam < thirty & esam > 0] <- "ESAM-"
...
Verify
``` {r}
full <- FetchData(seurat_object, vars = c("adt_CD150", "adt_CD135"))
mpp4 <- FetchData(test, vars = c("adt_CD150", "adt_CD135"))
countfull <- sum(full$adt_CD150 > 0 & full$adt_CD135 > 0)
countmpp4 <- sum(mpp4$adt_CD150 > 0 & mpp4$adt_CD135 > 0)
countfull
countmpp4
```
