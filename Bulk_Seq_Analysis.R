...
# sample selection
gsms <- paste0("13013123321313113333030303333133013111113210303331",
               "1301313332133131312333132333113133313223XXXXXXXXXX",
...
gs <- factor(sml)
groups <- make.names(c("WT","MDS SF3B1","MDS SRSF2","MDS"))
levels(gs) <- groups
...
plotDispEsts(ds, main="GSE114922 Dispersion Estimates")

# create histogram plot of p-values
hist(r$padj, breaks=seq(0, 1, length = 21), col = "grey", border = "white", 
     xlab = "", ylab = "", main = "GSE114922 Frequencies of padj-values")
...
par(mar=c(7,4,2,1))
boxplot(dat[,ord], boxwex=0.6, notch=T, main="GSE114922", ylab="lg(norm.counts)", outline=F, las=2, col=gs[ord])
...
# Assuming 'seurat_obj' is your Seurat object and 'res' is your DESeq2 result object
# And 'genes_of_interest' is a vector of gene names
genes_of_interest <- c("8737", "11035", "257397", "29110", "841")

# Subset the data to include only the genes of interest
data_subset <- dat[genes_of_interest, ]
group = kmeans(t(data_subset), centers = 3)$cluster
Heatmap(data_subset, column_split = gs, show_column_names = FALSE )
Heatmap(data_subset, column_order = sort(colnames(ds)), show_column_names = FALSE, column_split = 4)
cluster_columns = cluster_within_group(data_subset, group)
pheatmap(data_subset, annotation_col = sample_info, show_colnames = FALSE,)
